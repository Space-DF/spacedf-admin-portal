import { SVGProps } from '@/types';

export const Instagram = (props: SVGProps) => {
  return (
    <svg
      xmlns='http://www.w3.org/2000/svg'
      width='18'
      height='18'
      viewBox='0 0 18 18'
      fill='currentColor'
      {...props}
    >
      <path d='M12.75 1.5H5.25C3.18 1.5 1.5 3.18 1.5 5.25V12.75C1.5 14.82 3.18 16.5 5.25 16.5H12.75C14.82 16.5 16.5 14.82 16.5 12.75V5.25C16.5 3.18 14.82 1.5 12.75 1.5ZM9 12.75C6.93 12.75 5.25 11.07 5.25 9C5.25 6.93 6.93 5.25 9 5.25C11.07 5.25 12.75 6.93 12.75 9C12.75 11.07 11.07 12.75 9 12.75ZM13.0125 5.715C12.6 5.715 12.2625 5.3775 12.2625 4.965C12.2625 4.5525 12.6 4.215 13.0125 4.215C13.425 4.215 13.7625 4.5525 13.7625 4.965C13.7625 5.3775 13.425 5.715 13.0125 5.715Z' />
      <path d='M9 11.25C10.2426 11.25 11.25 10.2426 11.25 9C11.25 7.75736 10.2426 6.75 9 6.75C7.75736 6.75 6.75 7.75736 6.75 9C6.75 10.2426 7.75736 11.25 9 11.25Z' />
    </svg>
  );
};
